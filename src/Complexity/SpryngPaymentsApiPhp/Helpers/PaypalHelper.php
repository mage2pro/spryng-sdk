...
class PaypalHelper
{
    public static function validateInitializePaypalArguments(array $arguments)
    {
        ...
        if (!isset($arguments['details']['capture_now']))
        {
            throw new TransactionException("Capture now it not provided.". 209);
        }
        else
        {
            if (!is_bool($arguments['details']['capture_now']))
            {
                throw new TransactionException("Capture now is not a boolean", 209);
            }
        }
    }
}